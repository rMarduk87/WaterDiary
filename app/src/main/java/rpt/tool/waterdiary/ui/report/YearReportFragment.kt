...
import rpt.tool.waterdiary.databinding.FragmentYearReportBinding
...
class YearReportFragment : NavBaseFragment<FragmentYearReportBinding>(FragmentYearReportBinding::inflate) {
    ...
    private fun loadData(start_calendar2: Calendar, end_calendar2: Calendar) {
        ...
        for (i in lst_date.indices) {
            val arr_data: ArrayList<HashMap<String, String>> = 
                dh!!.getdata("tbl_drink_details", "DrinkDate like '%" + lst_date2[i] + "%'")
            ...
            for (t in lst_unique_date.indices) {
                val arr_data22: ArrayList<HashMap<String, String>> = dh!!.getdata(
                        "tbl_drink_details",
                ...
                if (arr_data22.size > 0) {
                    if (AppUtils.WATER_UNIT_VALUE.equals("ml",true)){
                        val c = arr_data[0]["TodayGoal"]!!.toFloat()
                        total_goal += c
                    }
                    else{
                        val c = arr_data[0]["TodayGoalOZ"]!!.toFloat()
                        total_goal += c
                    }
                }
            }
    ...
    fun showDayDetailsDialog(position: Int) {
        ...
        txt_date.text = dth!!.formateDateFromString("MMM-yyyy", "MMM yyyy", lst_date[position])
        txt_goal.text =
            ("" + lst_date_goal_val_2[position] + " " + AppUtils.WATER_UNIT_VALUE) + "/" + sh!!.get_string(
                R.string.day
            )
        txt_consumed.text =
            ("" + lst_date_val[position] + " " + AppUtils.WATER_UNIT_VALUE) + "/" + sh!!.get_string(
                R.string.day
            )

        val arr_data: ArrayList<HashMap<String, String>> = dh!!
            .getdata("tbl_drink_details", 
                "DrinkDate like '%" + lst_date2[position] + "%'")
        ...
        for (t in lst_unique_date.indices) {
            val arr_data2: ArrayList<HashMap<String, String>> = dh!!.getdata(
                "tbl_drink_details",
                "DrinkDate ='" + lst_unique_date[t] + "'"
            )
