...
import rpt.tool.waterdiary.databinding.FragmentWeekReportBinding
...
class WeekReportFragment : NavBaseFragment<FragmentWeekReportBinding>(FragmentWeekReportBinding::inflate) {
    ...
    var lst_week: ArrayList<String> = ArrayList()
 ...
 current_start_calendar : ${current_end_calendar!!.timeInMillis}"""
    ...
    private fun loadData(start_calendar2: Calendar, end_calendar2: Calendar) {
        ...
        for (i in lst_date.indices) {
            ...
            for (j in arr_data.indices) {
                if (AppUtils.WATER_UNIT_VALUE.equals("ml",true)) {
                    ...
                    val x = arr_data[j]["ContainerValueOZ"]!!.toFloat()
                    tot += x
        ...
        try {
            ...
            binding.txtAvgIntake.text = ("0 " + AppUtils.WATER_UNIT_VALUE).toString() + "/" + sh!!.get_string(
                R.string.day
            )
    ...
    fun showDayDetailsDialog(position: Int) {
        ...
        txt_date.text = dth!!
            .formateDateFromString("dd-MM-yyyy", "dd MMM", lst_date[position])
    ...
    private fun setData(count: Int) {
        ...
        if (binding.chartNew.data != null &&
            ...
            set1.color = MasterBaseAppCompatActivity.getThemeColor(mContext!!)
